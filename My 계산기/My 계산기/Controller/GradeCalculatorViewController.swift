...
class GradeCalculatorViewController: UIViewController{
    ...
    @IBAction func pressedAddButton(_ sender: UIButton) {
        ...
        if totalGradeInfo.count == 15 || loadedGradeInfo.count == 15 {
            createAlertMessage("과목 수 제한", "15개 이상의 과목을 입력할 수 없습니다.")    // 굳이 제한을 둬야하는지 생각 다시 해보기
            return
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if !loadedGradeInfo.isEmpty {
            ...
            if let credit = loadedGradeInfo[indexPath.row].credit, let grade = loadedGradeInfo[indexPath.row].grade {
                ...
                let gradeInDouble = cell.convertGradeStringToDouble(gradeString)
                totalGradeInfo[indexPath.row].grade = gradeInDouble
...
extension GradeCalculatorViewController: UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        highestPossibleGradeTextField.text = "\(selectedOption) 만점 ▼"
